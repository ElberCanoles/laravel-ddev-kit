#!/usr/bin/env bash
#
# setup.sh — Deja una máquina Ubuntu recién instalada lista para desarrollo
...
if ! grep -q "machine-setup/bin" "$HOME/.bashrc" 2>/dev/null; then
  printf '\n# helpers machine-setup (new-laravel, adopt-laravel, backup-projects)\nexport PATH="$PATH:%s/bin"\n' "$RAIZ_REPO" >> "$HOME/.bashrc"
